...
##        self.__doit()
    ...
    def __process_file(self,filepath):
        f = open(filepath,"rb")
        contents = f.read()
        for m in re.finditer(self.__pattern,contents):
            self.__found = True
            if self.__files_with_matches:
                ...
                span = m.span()
                self.__output_stream.write("{}:0x{:x}: {}\n".format(filepath,span[0],contents[span[0]:span[1]].decode()))
        f.close()
    ...
    def __doit(self):
        ...
        self.__found = False
        self.__pattern = re.compile(pattern.encode(),flags)
